<aside class="l-sidebar">
  <div class="b-about">
    <h2 class="b-about__title">О нас</h2>

    <img class="b-about__thumb" src="<?php echo get_theme_file_uri( 'assets/img/userfiles/about-ill.jpg' ); ?>" alt="О нас" />
    <div class="b-about__content b-text">
      <p>Веб-сайты, мобильные приложения, корпоративные интернет решения — всем этим наша компания занимается уже более 12 лет, предлагая услуги по консультации, разработке, внедрению и сопровождению различных проектов для наших клиентов.</p>
    </div>
  </div>

  <div class="b-sidebarBlock -style_news">
    <h2 class="b-sidebarBlock__title">Последние новости</h2>

    <div class="l-lastNews">
      <ul class="b-lastNews">
        <li class="b-lastNews__item">
          <a class="b-lastNews__thumbLink" href="#">
            <img class="b-lastNews__thumb" src="<?php echo get_theme_file_uri( 'assets/img/userfiles/lastNews_1-ill.jpg' ); ?>" alt="ideus_team at WebCampOdessa" />
          </a>

          <div class="b-lastNews__content">
            <h3 class="b-lastNews__name">
              <a class="b-lastNews__nameLink" href="#">Конференция #WebCamp 2015 в Одессе</a>
            </h3>
            <div class="b-lastNews__date">8 июля 2015</div>
            <div class="b-lastNews__descr b-text">
              <p>3-5 июля мы побывали на конференции WebCamp…</p>
            </div>
          </div>
        </li>

        <li class="b-lastNews__item">
          <a class="b-lastNews__thumbLink" href="#">
            <img class="b-lastNews__thumb" src="<?php echo get_theme_file_uri( 'assets/img/userfiles/lastNews_2-ill.png' ); ?>" alt="Запуск интерактивной библиотеки для детей под iOS «Візок казок»" />
          </a>

          <div class="b-lastNews__content">
            <h3 class="b-lastNews__name">
              <a class="b-lastNews__nameLink" href="#">Запуск интерактивной библиотеки для детей под iOS «Візок казок»</a>
            </h3>
            <div class="b-lastNews__date">19 июня 2015</div>
            <div class="b-lastNews__descr b-text">
              <p>Друзья! Мы спешим сообщить вам, что в…</p>
            </div>
          </div>
        </li>
      </ul>
    </div>
  </div>

  <div class="b-sidebarBlock -style_social">
    <h2 class="b-sidebarBlock__title">Присоединяйся к нам</h2>

    <ul class="b-socialMenu -style_sidebar">
      <li class="b-socialMenu__item -type_fb">
        <a class="b-socialMenu__link" href="https://www.facebook.com/ideus.interactive">Facebook</a>
      </li>
      <li class="b-socialMenu__item -type_twitter">
        <a class="b-socialMenu__link" href="https://twitter.com/iDeus_team">Twitter</a>
      </li>
      <li class="b-socialMenu__item -type_github">
        <a class="b-socialMenu__link" href="https://github.com/ideus-team">GitHub</a>
      </li>
    </ul>
  </div>
</aside>
